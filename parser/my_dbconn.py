...
def db_fill(url):
    try:
        ...
        cursor.execute(f"""SELECT id FROM dish_category_entity WHERE title = '{category}'""")
        if cursor.rowcount <= 0:
            cursor.execute(f"""INSERT INTO dish_category_entity (TITLE) VALUES ('{category}') RETURNING id""")
        ...
        for recipe in finalresult:
            ...
            insert_dish_query = f"""INSERT INTO dish_entity (TITLE, DESCRIPTION, DISH_CAT_ID) VALUES ('{dish_title}', '{dish_description}', '{category_id}') RETURNING id"""
            ...
            for product in recipe[1]:
                ...
                insert_recipe_query = f""" INSERT INTO recipes (dish_id, product_id) VALUES ('{dish_id}', '{product_id}')"""
